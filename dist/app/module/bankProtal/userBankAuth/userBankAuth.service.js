...
exports.UserBankAuthServices = void 0;
...
const userBankAuth_constant_1 = require("./userBankAuth.constant");
const formateBankName_1 = require("../../../utils/formateBankName");
const formatDate_1 = require("../../../utils/formatDate");
//Sign up User
const userBankRegister = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    const loginId = (0, loginIdGenerator_1.generateLoginId)(); //genarate 9 digit login id fir login perpose
    payload.loginId = loginId;
    console.log(payload);
    const isExistingBank = yield app_1.prisma.userBank.findFirst({
        where: {
            OR: [
                { email: payload.email },
    ...
    if (isExistingBank) {
        throw new AppError_1.default(http_status_codes_1.StatusCodes.CONFLICT, "User already exist please login, Thank you");
    }
    // Import the BankName enum from Prisma client
    // Ensure payload matches the expected type for Prisma
    const result = yield app_1.prisma.userBank.create({
...
const login = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const userBank = yield app_1.prisma.userBank.findFirst({
        where: {
            AND: [{ bankName }, { loginId }]
        }
    });
    if (!userBank) {
        throw new AppError_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, 'User not found Check your login id');
    }
    if (!userBank.isActive) {
        throw new AppError_1.default(500, 'Your account is not active now call finupsbd team thank you.');
    }
    if (userBank.isBlocked) {
        throw new AppError_1.default(http_status_codes_1.StatusCodes.FORBIDDEN, 'Your account is blocked after too many failed login attempts. Please contact support.');
    }
    if (userBank.password !== password) {
        ...
        yield app_1.prisma.userBank.update({
            where: { loginId },
            data: updateData
        });
        const mailSubject = "🔒 Security Alert: Your FinupsBD Account Has Been Locked";
        const bodyText = yield (0, userBankAuth_constant_1.mailBodyText)(userBank.loginId, (0, formateBankName_1.formatBankName)(userBank.bankName), (0, formatDate_1.formatDate)(userBank.blockedAt, 'ddd, MMM D, YYYY h:mm A', { locale: 'en-GB' }));
        if (attempts >= 3) {
            yield (0, sendEmail_1.default)(userBank.email, mailSubject, bodyText);
        }
        const triesLeft = Math.max(0, 3 - attempts);
        throw new AppError_1.default(http_status_codes_1.StatusCodes.UNAUTHORIZED, `Invalid credentials. ${triesLeft} attempt(s) remaining before your account is blocked.`);
    }
    const jwtPayload = {
    ...
    const userBankAccessToken = (0, tokenGenerate_1.accessTokenGenerate)(jwtPayload, '1d');
    const userBankRefreshToken = (0, tokenGenerate_1.refreshTokenGenerate)(jwtPayload, '365d');
    return {
        userBankAccessToken,
