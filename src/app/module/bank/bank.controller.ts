...
const createBankInfo = catchAsync(async (req, res) => {
  const payload = BankValidationSchema.bankInfoValidateSchema.parse(
...
const updateBookInfo = catchAsync(async (req, res) => {
  const payload = BankValidationSchema.updateBankInfoValidateSchema.parse(
    JSON.parse(req.body.data)
  );
  console.log(payload);
  const file = req.file;
  const result = await BankInfoService.updateBankInfo(
    payload as TBank,
    file,
    req.params.id
  );
